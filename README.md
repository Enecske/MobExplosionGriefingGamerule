...
It adds a custom gamerule, `mobExplosionGriefing` (`true` by default), which makes all mob explosions (except Withers') not destroy blocks. Other actions, like fireballs lighting fires, will still happen.

Additionally, certain actions can be controlled separately.
...
- By turning `turtleEggGriefing` off, mobs will not pathfind to turtle eggs and won't destroy them upon stepping on them.
